<template>
  <div class="taint-manage-optimized">
    <!-- 页面头部 -->
    <div class="page-header">
      <div class="header-left">
        <h1 class="page-title">污点管理 (优化版)</h1>
        <p class="page-description">管理Kubernetes节点污点，支持批量操作进度显示和撤销重做</p>
      </div>
      ...
      >
        <div class="taint-header">
          <div class="taint-key-value">
            ...
            <div class="taint-value">{{ Object.keys(template.taints || {}).length }} 个污点</div>
          </div>
        ...
        <div class="taint-content">
          <div v-if="template.taints && Object.keys(template.taints).length > 0" class="taint-tags">
            <el-tag
              v-for="(taintInfo, key) in template.taints"
              :key="key"
              size="small"
              class="taint-tag"
              :type="getTaintEffectType(taintInfo.effect)"
              :title="`${key}=${taintInfo.value}:${taintInfo.effect}`"
            >
              <span class="tag-key">{{ key }}</span>=<span class="tag-value">{{ taintInfo.value }}</span>:<span class="tag-effect">{{ taintInfo.effect }}</span>
            </el-tag>
          </div>
      ...
      <div class="apply-dialog-content">
        <div class="template-summary">
          ...
          <div class="template-details">
            ...
            <p><strong>污点数量:</strong> {{ Object.keys(selectedTemplate?.taints || {}).length }}</p>
          </div>
        </div>
...
const searchFilters = ref([
  ...
  { key: 'taints', label: '污点内容', type: 'input' },
  { key: 'effect', label: '污点效果', type: 'select', options: [
...
const taintStats = computed(() => {
  ...
  taintTemplates.value.forEach(template => {
    Object.values(template.taints || {}).forEach(taint => {
      stats.total++
      switch (taint.effect) {
        case 'NoSchedule':
          stats.noSchedule++
...
const filteredTaints = computed(() => {
  if (!searchKeyword.value) {
    return taintTemplates.value
  }
  
  const keyword = searchKeyword.value.toLowerCase()
  return taintTemplates.value.filter(template => {
    return template.name?.toLowerCase().includes(keyword) ||
           Object.keys(template.taints || {}).some(key => 
             key.toLowerCase().includes(keyword) ||
             String(template.taints[key].value).toLowerCase().includes(keyword) ||
             String(template.taints[key].effect).toLowerCase().includes(keyword)
           )
  })
})
...
const refreshData = async () => {
  try {
    ...
    ElMessage.error('获取污点模板失败: ' + error.message)
  } finally {
    loading.value = false
  }
}
...
const confirmApplyTemplate = async () => {
  ...
  try {
    ...
    const operationId = progressStore.startBatchOperation({
      type: 'batch_apply_taints',
      title: '批量应用污点',
      description: `正在为 ${selectedNodes.value.length} 个节点应用污点模板 "${selectedTemplate.value.name}"...`,
    ...
    const applyData = {
      ...
      operation: 'add',
      taints: selectedTemplate.value.taints || {}
    }
    
    // 执行API调用
    try {
      const result = await taintApi.applyTemplate(applyData)
      ...
      historyStore.addOperation({
        type: 'taints.batch_apply',
        description: `应用污点模板 "${selectedTemplate.value.name}" 到 ${selectedNodes.value.length} 个节点`,
        undoAction: async () => {
          // 实现撤销逻辑 - 批量删除刚添加的污点
          const deleteData = {
            cluster_name: clusterName,
            nodes: selectedNodes.value,
            keys: Object.keys(selectedTemplate.value.taints || {})
          }
      ...
      ElMessage.success(`成功为 ${selectedNodes.value.length} 个节点应用污点模板`)
