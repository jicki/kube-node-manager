...
-- +migrate Up
ALTER TABLE ansible_tasks ADD COLUMN IF NOT EXISTS timeout_seconds INTEGER DEFAULT 0;
ALTER TABLE ansible_tasks ADD COLUMN IF NOT EXISTS is_timed_out BOOLEAN DEFAULT false;
...
-- 添加索引，方便查询超时任务
CREATE INDEX IF NOT EXISTS idx_ansible_tasks_is_timed_out ON ansible_tasks (is_timed_out) WHERE is_timed_out = true;
