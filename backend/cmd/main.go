...
func setupRoutes(router *gin.Engine, handlers *handler.Handlers) {
	...
	nodes := protected.Group("/nodes")
	{
		...
		nodes.GET("/:cluster_id/:node_name", handlers.Node.GetByName)
		nodes.GET("/:cluster_id/stats", handlers.Node.GetStats)
	}
	
	labels := protected.Group("/labels")
	{
		labels.GET("/:cluster_id/:node_name", handlers.Label.GetNodeLabels)
		labels.POST("/:cluster_id/:node_name", handlers.Label.AddLabels)
		labels.DELETE("/:cluster_id/:node_name", handlers.Label.RemoveLabels)
	...
	{
		taints.GET("/:cluster_id/:node_name", handlers.Taint.GetNodeTaints)
		taints.POST("/:cluster_id/:node_name", handlers.Taint.AddTaints)
		taints.DELETE("/:cluster_id/:node_name", handlers.Taint.RemoveTaints)
